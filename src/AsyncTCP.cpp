...
int8_t AsyncClient::abort()
{
    if (_pcb)
    {
        ...
        _pcb = NULL;
        _try_connect_pcb = NULL;
    }
    else if (_try_connect_pcb)
    {
        _tcp_abort(_try_connect_pcb, _closed_slot);
        _try_connect_pcb = NULL;
    }
    return ERR_ABRT;
}
...
int8_t AsyncClient::_close()
{
    // ets_printf("X: 0x%08x\n", (uint32_t)this);
    int8_t err = ERR_OK;
    if (_pcb == NULL && _try_connect_pcb)
    {
        _pcb = _try_connect_pcb;
        _try_connect_pcb = NULL;
    }
    if (_pcb)
    {
